...
export class Prompter {
  constructor(agent, fp) {
    ...
    if (typeof chat === "string" || chat instanceof String) {
      chat = { model: chat };
      if (chat.model.includes("gemini")) chat.api = "google";
      else if (chat.model.includes("gpt") || chat.model.includes("o1"))
        chat.api = "openai";
      else if (chat.model.includes("claude")) chat.api = "anthropic";
      else if (chat.model.includes("huggingface/")) chat.api = "huggingface";
      else if (
        chat.model.includes("meta/") ||
        chat.model.includes("mistralai/") ||
        chat.model.includes("replicate/")
      )
        chat.api = "replicate";
      else if (
        chat.model.includes("groq/") ||
        chat.model.includes("groqcloud/")
      )
        chat.api = "groq";
      else if (chat.model.includes("qwen")) chat.api = "qwen";
      else chat.api = "ollama";
    }
    ...
    if (chat.api === "google")
      ...
      this.chat_model = new GroqCloudAPI(
    ...
    if (embedding === undefined) {
      if (chat.api !== "ollama") embedding = { api: chat.api };
      else embedding = { api: "none" };
  ...
  async initExamples() {
    try {
      ...
      if (convoResult.status === "rejected") {
        console.error(
          "Failed to load conversation examples:",
          convoResult.reason
        );
  ...
  async replaceStrings(
    ...
    if (prompt.includes("$SELF_PROMPT")) {
      let self_prompt = this.agent.self_prompter.on
