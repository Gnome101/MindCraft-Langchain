...
function parseCommandMessage(message) {
  ...
  for (let i = 0; i < args.length; i++) {
    ...
    switch (param.type) {
      ...
      case "BlockName":
      case "ItemName":
        if (arg.endsWith("plank")) arg += "s"; // catches common mistakes like "oak_plank" instead of "oak_planks"
      case "string":
        break;
      default:
        throw new Error(
...
export async function executeCommand(agent, message) {
  ...
  if (typeof parsed === "string")
    ...
    if (numArgs !== numParams(command))
      return `Command ${
        command.name
      } was given ${numArgs} args, but requires ${numParams(command)} args.`;
    else {
  ...
  for (let command of commandList) {
    ...
    if (command.params) {
      ...
      for (let param in command.params) {
        docs += `${param}: (${
          typeTranslations[command.params[param].type] ??
          command.params[param].type
        }) ${command.params[param].description}\n`;
      }
    }
  }
  return docs + "*\n";
