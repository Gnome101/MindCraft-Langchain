export class ActionManager {
  ...
  async _executeResume(actionLabel = null, actionFn = null, timeout = 10) {
    ...
    if (
      this.resume_func != null &&
      this.agent.isIdle() &&
      (!this.agent.self_prompter.on || new_resume)
    ) {
  ...
  async _executeAction(actionLabel, actionFn, timeout = 10) {
    ...
    try {
      ...
      let message =
        this._getBotOutputSummary() + "!!Code threw exception!!  Error: " + err;
      let interrupted = this.agent.bot.interrupt_code;
      this.agent.clearBotLogs();
      if (!interrupted && !this.agent.coder.generating) {
        this.agent.bot.emit("idle");
  ...
  _getBotOutputSummary() {
    ...
    if (output.length > MAX_OUT) {
      output = `Code output is very long (${
        output.length
      } chars) and has been shortened.\n
          First outputs:\n${output.substring(
            0,
            MAX_OUT / 2
          )}\n...skipping many lines.\nFinal outputs:\n ${output.substring(
        output.length - MAX_OUT / 2
      )}`;
    } else {
      output = "Code output:\n" + output;
    }
